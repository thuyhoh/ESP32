...
void Send_Data_PHP(String nhietdo , String doam  , String TTTB1 , String TTTB2 , String BienEF , String BienGH )
{
  ...
  http.begin("http://192.168.1.143:82/IOTDATABASE/ESPinsertdatabase.php");
   //http.begin("http://toannv10291.000webhostapp.com/ESPinsertdatabase.php");
...
void UpdateDieuKhien(String TTTB1, String TTTB2)
{
  HTTPClient http;
  http.begin("http://192.168.1.143:82/IOTDATABASE/UpdateDieukhienVDK.php");
  http.addHeader("Content-Type", "application/x-www-form-urlencoded");
  String httpRequestData = "RL1="   + String(TTTB1)
                           + "&RL2=" +  String(TTTB2)                  
                           + "";
  int httpResponseCode = http.POST(httpRequestData);
  if (httpResponseCode == 200)
...
void Read_DieuKhien_PHP()
{
  ...
  http.begin("http://192.168.1.143:82/IOTDATABASE/ESPdieukhien.php");
   //http.begin("http://toannv10291.000webhostapp.com/ESPdieukhien.php");
  http.addHeader("Content-Type", "application/x-www-form-urlencoded");  
  ...
  if (digitalRead(NUT1) == LOW)
  {
    ...
    while (1)
    {
      ...
      if (digitalRead(NUT1) == HIGH)
      {
        ...
        UpdateDieuKhien(String(TTTB1), String(TTTB2));
        last = millis();
        break;
      }
    }
